defmodule ElixirOfLife.BoardServerTest do
  ...
  test "add cell to board" do
    cell_coord = {1, 1}
    color = %{red: 100, green: 100, blue: 100}
    cell = %{coord: cell_coord, color: color, pattern: nil}
  ...
  test "add cells to board with block pattern" do
    cell_coord = {1, 1}
    color = %{red: 100, green: 100, blue: 100}
    cell = %{coord: cell_coord, color: color, pattern: "block"}
    expected_cells = MapSet.new([
      {1, 1}, {1, 2},
      {2, 1}, {2, 2}
    ])
    {:ok, updated_board} = BoardServer.add_cells(cell)
    assert MapSet.equal?(updated_board.alive_cells, expected_cells)
    assert Map.fetch(updated_board.alive_cell_colors, cell_coord) == {:ok, color}
  end
